  ...
  def seed(env)
    ...
    if env == 'production'
      ...
      conn.collection(:account_info).save({
        :_id => 'i_am_awesome', 'active' => true,  'max_call_rate' => 1_000_000, 'ips' => ['0.0.0.0']
      })

      # this user's account is disabled
      conn.collection(:account_info).save({
        :_id => 'i_am_lame',    'active' => false, 'max_call_rate' => 1_000, 'ips' => ['192.168.1.1']
      })

      # this user has not been seen, but will very quickly hit their limit
      conn.collection(:account_info).save({
        :_id => 'i_am_limited', 'active' => true, 'max_call_rate' =>     10, 'ips' => ['127.0.0.1', '192.168.1.1']
      })
      ...
      conn.collection(:account_info).save({
        :_id => 'i_am_busy',    'active' => true, 'max_call_rate' =>  1_000, 'ips' => ['127.0.0.1', '192.168.1.1']
      })
      conn.collection(:usage_info).save({
