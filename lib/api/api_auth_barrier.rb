  ...
  class ApiAuthBarrier
    ...
    if defined?(EM::Mongo::Cursor)
      ...
      def enqueue_mongo_request(handle, query)
        enqueue handle, Api.db.collection(handle).afirst(query)
      end
    else
      # em-mongo <= 0.3.6 makes us fake a deferrable response.
      def enqueue_mongo_request(handle, query)
        enqueue_acceptor(handle) do |acc|
          Api.db.collection(handle).afind(query){|resp| acc.succeed(resp.first) }
        end
      end
    end
    ...
    def charge_usage
      EM.next_tick do
        safely(env){ Api.db.collection(:usage_info).update({ :_id => usage_id },
